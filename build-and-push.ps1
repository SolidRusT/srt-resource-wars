...
<#
.SYNOPSIS
    Build and push Resource Wars game server Docker image to GitHub Container Registry

.DESCRIPTION
    Builds the Resource Wars Godot headless server Docker image and optionally pushes to GitHub Container Registry

.PARAMETER GitHub Container RegistryUsername
    GitHub Container Registry username (default: suparious)

.PARAMETER ImageName
    Docker image name (default: resource-wars)
...
Write-ColorOutput "`n=== Resource Wars - Game Server Docker Builder ===" $colors.Header
Write-ColorOutput "Building Docker image for Godot headless game server`n" $colors.Info
...
Write-ColorOutput "This may take 5-10 minutes (Godot export + dependencies)" $colors.Info
Write-Host ""
...
Write-ColorOutput "Checking if game server executable exists..." $colors.Info
docker run --rm $fullImageName ls -lh /app/resource-wars-server.x86_64
if ($LASTEXITCODE -ne 0) {
    Write-ColorOutput "⚠️  Image test failed, server executable not found" $colors.Warning
}
else {
    Write-ColorOutput "✅ Game server executable found" $colors.Success
}
Write-Host ""
...
if (-not $Push) {
    ...
    Write-Host "  1. Test the game server locally:" -ForegroundColor Yellow
    Write-Host "     docker run --rm -p 9999:9999/udp -p 9998:9998/tcp $fullImageName" -ForegroundColor Yellow
    Write-Host "     Connect game clients to localhost:9999" -ForegroundColor Yellow
    Write-Host ""
